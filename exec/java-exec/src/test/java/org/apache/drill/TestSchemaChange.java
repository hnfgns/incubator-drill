...
public class TestSchemaChange extends BaseTestQuery {
  ...
  public void testMultiFilesWithDifferentSchema() throws Exception {
    final String plan = BaseTestQuery.getFile("schemachange/multi.json").replace("[WORKING_PATH]", WORKING_PATH);

    testBuilder()
        .optionSettingQueriesForTestQuery("alter system set `planner.width.max_per_node`=1;")
        .physicalPlan(plan)
        .ordered()
        .baselineColumns("a", "b")
        .baselineValues(1L, null)
        .baselineValues(null, true)
        .baselineValues(null, false)
        .build()
        .run();
  }
}
