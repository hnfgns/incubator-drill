...
public class CachingPersistentStoreProvider extends BasePersistentStoreProvider {
  static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(CachingPersistentStoreProvider.class);
  ...
  public <V> PersistentStore<V> getStore(final PersistentStoreConfig<V> config) throws StoreException {
    final PersistentStore<?> store = storeCache.get(config);
    if (store == null) {
      final PersistentStore<?> newStore = provider.getStore(config);
