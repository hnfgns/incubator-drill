...
public class MapVector extends AbstractContainerVector {
  ...
  public int getValueCapacity() {
    ...
    return natural.min(getVectors()).getValueCapacity();
  }
  ...
  public void load(SerializedField metadata, DrillBuf buf) {
    ...
    for (SerializedField fmd : fields) {
      ...
      ValueVector v = getVector(fieldDef.getLastName());
      if (v == null) {
        // if we arrive here, we didn't have a matching vector.
        v = TypeHelper.getNewVector(fieldDef, allocator);
        putVector(fieldDef.getLastName(), v);
      }
  ...
  public class Accessor implements ValueVector.Accessor{
    ...
    public Object getObject(int index) {
      ...
      for (String child:getChildFieldNames()) {
        Object value = getVector(child).getAccessor().getObject(index);
        if (value != null) {
          vv.put(child, value);
        }
      }
